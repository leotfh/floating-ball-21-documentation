...
If the program seems to not flash correctly after pressing the "Build & Deploy" button in the pop-up, the most likely culprit are the configuration parameters. Ensure that the "Build action" is set to "Build, load and run". This option seems to reset itself to "Build" especially, when merging Simulink Models using Git.
...
If the height sensor does not seem to be responding, ensure that it is properly connected to the Arduino and that the cables are fine. If nothing else works, try replacing the sensor as it might be defective due to the ball hitting the sensor.